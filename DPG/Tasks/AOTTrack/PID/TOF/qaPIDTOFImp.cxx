...
#undef particleCase
  ...
  void init(o2::framework::InitContext& context)
  {
    ...
    // LOG(info) << "Initializing resolution params:";
    // std::unordered_map<std::string, float> pars;
    // o2::aod::pidtof::TOFResponse::parameters.setResolutionParametrization(pars);
    // LOG(info) << "Parametrization at init time:";
    // o2::aod::pidtof::TOFResponse::parameters.printFullConfig();
  }
  ...
  using TrackCandidates = soa::Join<aod::Tracks, aod::TracksExtra, aod::TrackSelection,
                                    ...
                                    aod::pidTOFFlags>;
  using TrackCandidates2 = soa::Join<aod::TracksIU, aod::TracksExtra, aod::TrackSelection,
                                     aod::pidEvTimeFlags, aod::TOFSignal, aod::TOFEvTime,
                                     aod::pidTOFFlags>;
  ...
  using TrkPID = soa::Join<TrackCandidates2,
  ...
  void process2(TrkPID const& tracks, aod::Collisions const&)
  {
    ...
    for (const auto& t : tracksWithPid) {
      ...
      // if (!t.isGlobalTrack()) {
      //   continue; // Skipping unselected tracks
      // }
...
#define makeProcessFunction(inputPid, particleId)                                                                          \
  void processFull##particleId(CollisionCandidate const& /*collision*/,                                                    \
                               soa::Filtered<TrackCandidates> const& /*tracks*/)                                           \
  {                                                                                                                        \
    /* auto tracksWithPid = soa::Attach<TrackCandidates, aod::TOFNSigma##inputPid, aod::TOFExpSigma##inputPid>(tracks); */ \
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  ...
  return WorkflowSpec{adaptAnalysisTask<tofPidQaImp>(cfgc)};
}
